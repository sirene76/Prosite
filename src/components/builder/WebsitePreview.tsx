...
(function () {
  ...
  const findSectionTarget = (id, selector) => {
    const trimmedId = typeof id === 'string' ? id.trim() : '';
    const trimmedSelector = typeof selector === 'string' ? selector.trim() : '';
    let target = null;

    if (trimmedId) {
      const escapedId = escapeSelector(trimmedId.replace(/^#/, ''));
      const candidates = [
        '[data-section="' + escapedId + '"]',
        '[data-section-id="' + escapedId + '"]',
        '[data-builder-section="' + escapedId + '"]',
      ...
      if (!target) {
        const byId = document.getElementById(trimmedId.replace(/^#/, ''));
        if (byId) {
          target = byId;
        }
      }
      ...
      if (!target) {
        const fallback = normaliseSelector(trimmedId);
        if (fallback) {
          try {
    ...
    if (!target && trimmedSelector) {
      try {
        target = document.querySelector(trimmedSelector);
      } catch (error) {
        target = null;
      }
    }
  ...
  const scrollToSection = (id, selector) => {
    const target = findSectionTarget(id, selector);
  ...
  const updateField = (key, value) => {
    ...
    const escapedKey = escapeSelector(key.trim());
    let nodes = [];
    try {
      nodes = Array.from(document.querySelectorAll('[data-field="' + escapedKey + '"]'));
    } catch (error) {
      nodes = [];
    }
    ...
    const resolvedValue = resolveFieldValue(value);

    nodes.forEach((node) => {
      const attr = node.getAttribute('data-field-attr');
      const mode = node.getAttribute('data-field-mode');

      if (attr === 'background-image') {
        node.style.backgroundImage = resolvedValue ? 'url(' + resolvedValue + ')' : '';
        return;
      }

      if (attr) {
        node.setAttribute(attr, resolvedValue);
        return;
      }

      if (mode === 'html') {
        node.innerHTML = resolvedValue;
        return;
      }
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (data.type) {
      ...
      case 'scrollTo':
      case 'scroll-to':
        scrollToSection(data.id || data.sectionId, data.selector || data.anchor);
        break;
      case 'updateField':
        updateField(data.key, data.value);
        break;
      default:
        break;
    }
